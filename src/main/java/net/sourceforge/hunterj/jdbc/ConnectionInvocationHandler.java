...
/**
 * <code>ConnectionInvocationHandler</code> is provided to <code>java.lang.reflect.Proxy</code> 
 * to create a proxy to a <code>java.sql.Connection</code>. The {@link #invoke(Object, Method, Object[])}
 * method monitors calls to the target <code>Connection</code>. When a <code>java.sql.Statement
		...
		throws Throwable
	{
		...
		if(result instanceof java.sql.Statement) {	
			Statement proxyStatement = (Statement) Proxy.newProxyInstance(
					...
					new StatementInvocationHandler(result, this.getQueryContext()));
